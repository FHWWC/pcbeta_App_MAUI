...
public partial class ThreadListViewModel : ObservableObject
{
    ...
    private string? _lastThreadListXml = string.Empty;
    ...
    /// </summary>
    public async Task InitializeAsync(string boardId, string forumName, int pageNum = 1)
    {
    ...
    public async Task LoadThreadsAsync()
    {
        ...
        try
        {
            ...
            // ✅ 改进：从 ApiService 获取保存的 XML 内容来检测登录状态
            _lastThreadListXml = _apiService.LastPageContent;
            DetectLoginStatus(_lastThreadListXml);
    ...
    public async Task SelectThreadAsync(ThreadInfo thread)
    {
        ...
        try
        {
            var parameters = new Dictionary<string, object>
            {
                ...
                { "forumId", _currentBoardId ?? string.Empty },
                                { "boardName", BoardName }
            };

            Debug.WriteLine($"📝 SelectThreadAsync - threadId: {thread.Id}, forumId: {_currentBoardId}");
    ...
    public void ToggleStickyExpanded()
    {
        IsStickyExpanded = !IsStickyExpanded;
         ...
         /// <summary>
         /// Post new thread - check login status and navigate accordingly
         /// </summary>
         [RelayCommand]
         public async Task PostNewThreadAsync()
         {
             Debug.WriteLine($"[PostNewThread] 用户点击发新帖按钮，登录状态: {_isUserLoggedIn}");

             // 检查登录状态
             if (!_isUserLoggedIn)
             {
                 Debug.WriteLine("[PostNewThread] 用户未登录，重定向到登录页面");
                 await _navigationService.NavigateToAsync("login");
                 return;
             }

             // 用户已登录，导航到发帖页面
             Debug.WriteLine($"[PostNewThread] 用户已登录，导航到发帖页面: boardId={_currentBoardId}, boardName={BoardName}");

             var parameters = new Dictionary<string, object>
             {
                 { "boardId", _currentBoardId ?? string.Empty },
                 { "boardName", BoardName }
             };

             await _navigationService.NavigateToAsync("postnewthread", parameters);
         }

         /// <summary>
         /// Detects login status from the thread list XML response
         /// If ">登录</a>" is found, user is NOT logged in
         /// </summary>
