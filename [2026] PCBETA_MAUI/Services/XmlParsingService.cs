...
public partial class XmlParsingService
{
    ...
    /// 
    /// HTML 结构示例：
    /// <li class="pbw" id="2070067">
    ///   <h3 class="xs3">
    ///     <a href="...viewthread&tid=2070067..." target="_blank">标题</a>
    ///   </h3>
    ///   <p class="xg1">6 个回复 - 37 次查看</p>
    ///   <p>预览内容...</p>
    ///   <p>
    ///     <span>2026-4-30 12:35</span>
    ///     <span><a href="...space-uid-...">作者名</a></span>
    ///     <span><a href="...forum-569-1..." class="xi1">论坛名</a></span>
    ///   </p>
    /// </li>
    /// </summary>
    public List<SearchThreadInfo> ParseSearchResultsFromHtml(string html)
    {
        var results = new List<SearchThreadInfo>();

        try
        {
            // 提取所有搜索结果项：<li class="pbw" id="...">...</li>
            var liPattern = @"<li[^>]*class=""[^""]*pbw[^""]*""[^>]*id=""(\d+)""[^>]*>(.*?)</li>";
            var liMatches = Regex.Matches(html, liPattern, RegexOptions.IgnoreCase | RegexOptions.Singleline);

            Debug.WriteLine($"🔍 搜索结果解析：找到 {liMatches.Count} 个搜索结果");

            foreach (Match liMatch in liMatches)
            {
                try
                {
                    if (liMatch.Groups.Count < 3)
                        continue;

                    var threadId = liMatch.Groups[1].Value.Trim();
                    var liContent = liMatch.Groups[2].Value;

                    // ========== 1. 提取 URL 和标题 ==========
                    // 模式：<a href="...viewthread..." target="_blank">标题文本</a>
                    // 标题可能包含嵌套的 HTML 标签如 <strong>, <font> 等
                    var titlePattern = @"<a[^>]*href=""([^""]*)viewthread[^""]*""[^>]*target=""_blank""[^>]*>(.+?)</a>";
                    var titleMatch = Regex.Match(liContent, titlePattern, RegexOptions.IgnoreCase | RegexOptions.Singleline);

                    if (!titleMatch.Success)
                        continue;

                    var url = HtmlDecode(titleMatch.Groups[1].Value.Trim());
                    var titleHtml = titleMatch.Groups[2].Value;

                    // 移除标题中的所有 HTML 标签（<strong>, <font>, 等）
                    var title = Regex.Replace(titleHtml, @"<[^>]+>", "");
                    title = HtmlDecode(title).Trim();

                    // ========== 2. 提取回复数和查看数 ==========
                    // 模式：<p class="xg1">6 个回复 - 37 次查看</p>
                    var statsPattern = @"<p[^>]*class=""[^""]*xg1[^""]*""[^>]*>(\d+)\s*个回复\s*-\s*(\d+)\s*次查看</p>";
                    var statsMatch = Regex.Match(liContent, statsPattern, RegexOptions.IgnoreCase);

                    int replies = 0;
                    int views = 0;
                    if (statsMatch.Success)
                    {
                        int.TryParse(statsMatch.Groups[1].Value, out replies);
                        int.TryParse(statsMatch.Groups[2].Value, out views);
                    }

                    // ========== 3. 提取预览内容 ==========
                    // 提取第一个不带 class="xg1" 的 <p> 标签内容
                    var previewPattern = @"<p class=""xg1"">[\s\S]*?</p>\s*<p[^>]*>([\s\S]*?)</p>\s*<p>";
                    var previewMatch = Regex.Match(liContent, previewPattern, RegexOptions.IgnoreCase);

                    string previewContent = string.Empty;
                    if (previewMatch.Success)
                    {
                        previewContent = HtmlDecode(previewMatch.Groups[1].Value.Trim());
                        previewContent= Regex.Replace(previewContent, "<.*?>", "");
                    }

                    // ========== 4. 提取发布时间 ==========
                    // 模式：<span>2026-4-30 12:35</span>
                    var timePattern = @"<span[^>]*>(\d{4}-\d{1,2}-\d{1,2}\s+\d{1,2}:\d{2})</span>";
                    var timeMatch = Regex.Match(liContent, timePattern, RegexOptions.IgnoreCase);
                    ...
                    // ========== 5. 提取作者名 ==========
                    // 模式：<a href="...space-uid-...">作者名</a>
                    var authorPattern = @"<a[^>]*href=""[^""]*space-uid-[^""]*""[^>]*>([^<]+)</a>";
                    var authorMatch = Regex.Match(liContent, authorPattern, RegexOptions.IgnoreCase);

                    string author = string.Empty;
                    if (authorMatch.Success)
                    {
                        author = HtmlDecode(authorMatch.Groups[1].Value.Trim());
                    }

                    // ========== 6. 提取论坛名称 ==========
                    // 模式：<a href="...forum-569-1..." class="xi1">Windows 高级资源区</a>
                    var forumPattern = @"<a[^>]*href=""[^""]*forum-[^""]+""[^>]*class=""[^""]*xi1[^""]*""[^>]*>([^<]+)</a>";
                    var forumMatch = Regex.Match(liContent, forumPattern, RegexOptions.IgnoreCase);

                    string forumName = string.Empty;
                    if (forumMatch.Success)
                    {
                        forumName = HtmlDecode(forumMatch.Groups[1].Value.Trim());
                    }

                    // ========== 7. 创建搜索结果对象 ==========
                    var searchResult = new SearchThreadInfo
                    {
                        Id = threadId,
                        Url = url,
                        Title = title,
                    ...
                    Debug.WriteLine($"✅ 搜索结果 #{threadId}: 标题='{title.Substring(0, Math.Min(30, title.Length))}...' 作者='{author}' 回复={replies} 查看={views}");
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"❌ 解析单条搜索结果出错: {ex.Message}");
                    continue;
                }
            }

            Debug.WriteLine($"📊 搜索结果解析完成：共 {results.Count} 条结果");
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"❌ 搜索结果HTML解析错误: {ex.Message}");
        }

        return results;
    }

    public string GetSearchID(string html)
    {
        var pattern = $"searchid=(.*?)&";
        var searchidMatch = Regex.Match(html, pattern, RegexOptions.IgnoreCase | RegexOptions.Singleline);

        if (searchidMatch.Success)
        {
            return searchidMatch.Groups[1].Value;
        }
    ...
    /// </summary>
    public ThreadContent ParseThreadContent(string xmlResponse, int page = 1)
    {
        ...
        try
        {
            if (xmlResponse.Contains("<strong>登录</strong>"))
            {
                ...
                content = ExtractThreadContentFromHtml(xmlResponse, page);
            }
            else
            {
                var doc = XDocument.Parse(xmlResponse);
                ...
                // Parse HTML to extract thread content
                content = ExtractThreadContentFromHtml(htmlContent, page);
            }
        }
        catch (Exception ex)
        {
            ...
            foreach (var categoryDef in categoryDefinitions)
            {
                ...
                if (categoryMatch.Success)
                {
                    ...
                    if (sections.Count > 0)
                    {
                        ...
                        else if (categoryId == "533")//fg_533: PCBeta AI 论坛 - SVG图标转换为本地PNG
                        {
                            sections.ForEach(p =>
                            {
                                if (p.LogoUrl.Contains("common_525_icon"))
                                {
                                    p.LogoUrl = "common_525_icon.png";
                                }
                            });
                        }
    ...
    private List<ForumSection> ExtractSectionsFromCategoryHtml(string categoryHtml, string categoryId)
    {
        ...
        try
        {
            ...
            if (categoryId == "196" || categoryId == "533")
            {
                // fg_196 和 fg_533: col_0 single-column layout - extract from <tr> rows
    ...
    /// </summary>
    private ThreadContent ExtractThreadContentFromHtml(string htmlContent, int currentPage = 1)
    {
        var content = new ThreadContent
        {
            Id = "",
        ...
        try
        {
			...
			if (!string.IsNullOrEmpty(threadOwnerContent))
			{
			...
			//  新增：提取编辑按钮URL - 用于"编辑楼主发帖"功能
			//  HTML结构：<a class="editp" href="forum.php?mod=post&action=edit&fid=16&tid=2069510&pid=57952327&page=1">编辑</a>
			if (!string.IsNullOrEmpty(threadOwnerContent))
			{
				var editButtonPattern = @"<a[^>]*class=""editp""[^>]*href=""([^""]+)""[^>]*>";
				var editButtonMatch = Regex.Match(threadOwnerContent, editButtonPattern, RegexOptions.IgnoreCase);
				if (editButtonMatch.Success)
				{
					var editUrl = HtmlDecode(editButtonMatch.Groups[1].Value.Trim());
					// 构建完整的编辑URL，添加 &inajax=1 参数
					if (!editUrl.Contains("inajax"))
					{
						editUrl = $"https://bbs.pcbeta.com/{editUrl}&inajax=1";
					}
					content.CanEditOp = true;
					content.EditOpUrl = editUrl;
					Debug.WriteLine($"✏️ 提取到编辑按钮URL (可编辑): {editUrl.Substring(0, Math.Min(60, editUrl.Length))}...");
				}
				else
				{
					content.CanEditOp = false;
					content.EditOpUrl = null;
					Debug.WriteLine($"❌ 未找到编辑按钮，楼主发帖不可编辑");
				}
			}
            ...
            if (!string.IsNullOrEmpty(htmlContent) && !string.IsNullOrEmpty(threadOwnerContent))
            {
            ...
            var replies = ExtractRepliesFromHtml(htmlContent, isFirstPage);
            ...
            //  新增：提取分页信息
            var paginationInfo = ExtractPaginationInfo(htmlContent);
            if (paginationInfo != null)
            {
                content.CurrentPage = paginationInfo.CurrentPage;
                content.TotalPages = paginationInfo.TotalPages;
                Debug.WriteLine($"📄 分页信息: 当前第 {content.CurrentPage} 页，共 {content.TotalPages} 页");
            }

                Debug.WriteLine($" 已提取帖子内容 - 标题: {content.Title}, 作者: {content.Author}, 元素数: {content.ContentElements.Count}, 评论数: {content.Comments?.Count ?? 0}, 评分数: {content.RatingSummary?.TotalRatingCount ?? 0}, 回帖数: {content.ReplyList?.Count ?? 0}, 分页: {content.CurrentPage}/{content.TotalPages}");

                //  新增：提取楼主的评分URL - 用于"评分楼主"功能
                //  改进：从 onclick 属性中提取URL，而不是 href 属性
                //  HTML结构：onclick="showWindow('rate', 'forum.php?mod=misc&amp;action=rate&amp;tid=XXXX&amp;pid=XXXX', ...)"
                if (!string.IsNullOrEmpty(threadOwnerContent))
                {
                    var ratingUrlPattern = @"onclick=""[^""]*'(forum\.php\?[^']*mod=misc[^']*action=rate[^']*)";
                    var ratingUrlMatch = Regex.Match(threadOwnerContent, ratingUrlPattern, RegexOptions.IgnoreCase);
                    if (ratingUrlMatch.Success)
                    {
                        content.RatingUrl = HtmlDecode(ratingUrlMatch.Groups[1].Value.Trim());
                        Debug.WriteLine($"✅ 提取到楼主评分URL: {content.RatingUrl}");
                    }
                }

                //  新增：提取楼主的post ID - 用于评分时的身份标识
                //  HTML结构：<div id="post_57934040" ...>...</div>
                if (!string.IsNullOrEmpty(threadOwnerContent))
                {
                    var postIdPattern = @"<div[^>]*id=""post_(\d+)""[^>]*>";
                    var postIdMatch = Regex.Match(threadOwnerContent, postIdPattern, RegexOptions.IgnoreCase);
                    if (postIdMatch.Success)
                    {
                        content.AuthorPostId = postIdMatch.Groups[1].Value.Trim();
                        Debug.WriteLine($"✅ 提取到楼主的post ID: {content.AuthorPostId}");
                    }
                }
            }
            catch (Exception ex)
    ...
    private List<ContentElement> ParseContentBlock(string htmlContent, string? horizontalGroupId, string? suggestedFileName = null, bool isTableCell = false, string? fullHtmlContent = null)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < tokens.Count; i++)
            {
                ...
                // 处理引用块（支持 <div class="quote"><blockquote> 和 <blockquote> 两种格式）
                if (trimmedToken.Contains("class=\"quote\"") || trimmedToken.StartsWith("<blockquote", StringComparison.OrdinalIgnoreCase))
                {
                    var quoteElement = ExtractQuoteBlockquoteInfo(trimmedToken);
                    if (quoteElement != null)
                    {
                        quoteElement.HorizontalGroupId = horizontalGroupId;
    ...
    /// <summary>
    ///  新增：从 <div class="quote"><blockquote> 或 <blockquote> 结构中提取引用信息
    /// 支持的格式（基于 forum.xml 真实数据分析）：
    /// 
    /// 变体1：完整格式 - 嵌套链接 + 时间戳
    ///   <div class="quote"><blockquote><font size="2"><a><font color="#999999">引用者 发表于 时间</font></a></font><br>内容</blockquote></div>
    /// 
    /// 变体2：简化格式 - 仅内容
    ///   <div class="quote"><blockquote>内容</blockquote></div>
    /// 
    /// 变体3：直接嵌套 - 无链接包装
    ///   <div class="quote"><blockquote><font color="#999999">引用者 发表于 时间</font>内容</blockquote></div>
    /// 
    /// 变体4：另一种完整格式
    ///   <div class="quote"><blockquote><font size="2"><a><font color="#999999">引用者 发表于 时间</font></a></font><br>内容</blockquote></div>
    /// 
    ...
    /// 
    /// 提取内容：
    /// 1. 引用者名字：从 <font color="#999999"> 中提取
    /// 2. 引用时间：从 "发表于 YYYY-M-D HH:MM" 中提取
    /// 3. 引用内容：blockquote 中除了时间戳外的所有内容
    /// </summary>
    private ContentElement? ExtractQuoteBlockquoteInfo(string quoteHtml)
    {
        try
        {
            Debug.WriteLine($"📝 开始解析引用块: {quoteHtml.Substring(0, Math.Min(80, quoteHtml.Length))}...");

            // 步骤1：提取完整的 blockquote 内容（支持所有 5 种格式）
            var blockquotePattern = @"<blockquote[^>]*>(.*?)</blockquote>";
            var blockquoteMatch = Regex.Match(quoteHtml, blockquotePattern, RegexOptions.IgnoreCase | RegexOptions.Singleline);

            if (!blockquoteMatch.Success)
            {
                Debug.WriteLine("⚠️ 未找到 blockquote 标签，格式可能不支持");
                return null;
            }

            var blockquoteContent = blockquoteMatch.Groups[1].Value;
            Debug.WriteLine($"✅ 提取 blockquote 内容: 长度={blockquoteContent.Length}");

            // 步骤2：提取引用者和时间信息（支持多种嵌套方式）
            string quoterName = "";
            string quoteTime = "";

            // 模式可匹配：
            // - <font size="2"><a><font color="#999999">XXX 发表于 YYYY-M-D HH:MM</font></a></font>
            // - <font color="#999999">XXX 发表于 YYYY-M-D HH:MM</font>
            var headerPattern = @"<a href.*?>([^<]+?)\s+发表于\s+(\d{4}-\d{1,2}-\d{1,2}\s+\d{1,2}:\d{2})</a>";
            var headerMatch = Regex.Match(blockquoteContent, headerPattern, RegexOptions.IgnoreCase);

            if (headerMatch.Success)
            {
                quoterName = HtmlDecode(headerMatch.Groups[1].Value.Trim());
                quoteTime = headerMatch.Groups[2].Value.Trim();
                Debug.WriteLine($"✅ 提取引用者: {quoterName}, 时间: {quoteTime}");
            }
            else
            {
                Debug.WriteLine("ℹ️ 未找到引用者/时间信息（可能是简化格式）");
            }

            // 步骤3：提取引用内容（删除时间戳部分和所有 HTML 标签）
            string quoteContent = blockquoteContent;

            // 移除时间戳部分（支持所有嵌套方式）
            quoteContent = Regex.Replace(quoteContent, @"<font[^>]*size=""2""[^>]*>.*?</font>", "", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            quoteContent = Regex.Replace(quoteContent, @"<font[^>]*color=""#999999""[^>]*>.*?</font>", "", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            quoteContent = Regex.Replace(quoteContent, @"<a[^>]*>.*?</a>", "", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            quoteContent = Regex.Replace(quoteContent, @"<br\s*/?>\s*", "\n", RegexOptions.IgnoreCase);

            // ✅ 修复：分离 HTML 实体解码和标签删除，避免 StripHtmlTags 中的 \s+ 正则删除 \n
            // 步骤1：解码 HTML 实体
            quoteContent = HtmlDecode(quoteContent);
            // 步骤2：仅删除 HTML 标签，不规范化空白符（保留 \n）
            quoteContent = Regex.Replace(quoteContent, @"<[^>]+>", "");

            // 步骤3：规范化空白符（多个连续空格 → 单个空格，但保留换行符）
            quoteContent = Regex.Replace(quoteContent, @"[ \t]+", " ");  // 仅规范化空格和制表符
            quoteContent = Regex.Replace(quoteContent, @" *\n *", "\n");  // 移除换行前后的多余空格
            quoteContent = quoteContent.Trim();

            if (!string.IsNullOrEmpty(quoteContent))
            {
                Debug.WriteLine($"✅ 提取引用内容: {quoteContent.Substring(0, Math.Min(60, quoteContent.Length))}...");
            }
            else
            {
                Debug.WriteLine("⚠️ 引用内容为空（仅有时间戳，无内容）");
            }

            // 步骤4：构建引用头信息（用于 UI 显示）
            var quoteTitle = "";
            if (!string.IsNullOrEmpty(quoterName) && !string.IsNullOrEmpty(quoteTime))
            {
                quoteTitle = $"{quoterName} 发表于 {quoteTime}";
            }
            else if (!string.IsNullOrEmpty(quoterName))
            {
                quoteTitle = quoterName;
            }

            // 步骤5：如果既没有内容也没有时间戳信息，返回 null（无法识别的格式）
            if (string.IsNullOrEmpty(quoteContent) && string.IsNullOrEmpty(quoteTitle))
            {
                Debug.WriteLine("❌ 无法从引用块中提取任何有效信息");
                return null;
            }

            // 步骤6：创建并返回引用元素
            var quoteElement = new ContentElement
            {
                Type = ContentElementType.Quote,
                Text = quoteContent,  // 可能为空（变体2中仅有时间戳）
                Title = quoteTitle
            };

            // 步骤7：在 Attributes 中存储结构化的引用信息
            if (!string.IsNullOrEmpty(quoterName) || !string.IsNullOrEmpty(quoteTime))
            {
                quoteElement.Attributes = new Dictionary<string, string>();
                if (!string.IsNullOrEmpty(quoterName))
                    quoteElement.Attributes["quoter"] = quoterName;
                if (!string.IsNullOrEmpty(quoteTime))
                    quoteElement.Attributes["time"] = quoteTime;
            }

            Debug.WriteLine($"✅ 引用块解析完成 [Type: {(string.IsNullOrEmpty(quoterName) ? "简化" : "完整")}] Title='{quoteTitle}' Text='{(string.IsNullOrEmpty(quoteContent) ? "[空]" : quoteContent.Substring(0, Math.Min(30, quoteContent.Length)))}'");
            return quoteElement;
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"❌ 提取引用块错误: {ex.Message}");
    ...
    /// <summary>
    /// 新增：从编辑页面HTML中提取已有的附件（图片和普通附件）
    /// 
    /// 支持两种附件类型：
    /// 1. 图片附件：id="imageattach****"
    /// 2. 普通附件：id="attachname****"
    /// 
    /// 返回值：(imageAttachments, normalAttachments)
    /// </summary>
    public (List<Models.UploadedFileInfo> images, List<Models.UploadedFileInfo> attachments) ExtractAttachmentsFromEditPageHtml(string htmlContent)
    {
        var imageAttachments = new List<Models.UploadedFileInfo>();
        var normalAttachments = new List<Models.UploadedFileInfo>();

        try
        {
            // ========== 提取图片附件 ==========
            // 图片附件格式：<a href="..." title="..." id="imageattach4621637">
            var imagePattern = @"title=""([^""]*)""[^>]*id=""imageattach(\d+)""";
            var imageMatches = Regex.Matches(htmlContent, imagePattern, RegexOptions.IgnoreCase);

            Debug.WriteLine($"🖼️ 找到 {imageMatches.Count} 个图片附件");
            ...
            foreach (Match match in imageMatches)
            {
                if (match.Groups.Count >= 3)
                {
                    ...
                    var pattern = $@"imageattach{attachmentId}.*?<img src=""(.*?)"" id=""image_{attachmentId}""";
                    var match_imgPath = Regex.Match(htmlContent, pattern, RegexOptions.IgnoreCase);
                    if (match_imgPath.Success)
                    {
                        imgPath = "https://bbs.pcbeta.com/"+match_imgPath.Groups[1].Value;
                    }

                    //图片没有售价和阅读权限
                    var imageInfo = new Models.UploadedFileInfo
                    {
                        AttachmentId = attachmentId,
                        FileName = fileName,
                        FileSize = string.Empty, // 可选：如果需要，可从其他地方提取
                        Description = ExtractAttachmentMetadata(htmlContent, attachmentId, "description"),
                        ReadPerm = ExtractAttachmentMetadata(htmlContent, attachmentId, "readperm") ?? "0",
                        Price = ExtractAttachmentMetadata(htmlContent, attachmentId, "price") ?? "0",
                        LocalPath = imgPath
                    };

                    imageAttachments.Add(imageInfo);
                    Debug.WriteLine($"  ✅ 图片: ID={attachmentId}, 名称={fileName}");
                }
            }

            // ========== 提取普通附件 ==========
            // 普通附件格式：id="attachname****"
            var attachPattern = @"id=""attachname(\d+)""";
            var attachMatches = Regex.Matches(htmlContent, attachPattern, RegexOptions.IgnoreCase);

            Debug.WriteLine($"📎 找到 {attachMatches.Count} 个普通附件");

            foreach (Match match in attachMatches)
            {
                if (match.Groups.Count >= 2)
                {
                    string attachmentId = match.Groups[1].Value;
                    string fileName = ExtractAttachmentFileName(htmlContent, attachmentId);

                    var attachInfo = new Models.UploadedFileInfo
                    {
                        AttachmentId = attachmentId,
                        FileName = fileName,
                    ...
                    normalAttachments.Add(attachInfo);
                    Debug.WriteLine($"  ✅ 附件: ID={attachmentId}, 名称={fileName}");
                }
            }

            Debug.WriteLine($"✅ 提取附件完成: {imageAttachments.Count} 个图片, {normalAttachments.Count} 个普通附件");
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"❌ 提取附件错误: {ex.Message}");
        }

        return (imageAttachments, normalAttachments);
    }

    /// <summary>
    /// 辅助方法：从HTML中提取附件的指定元数据
    /// 查找格式：name="attachnew[****][fieldName]" value="..."
    /// </summary>
    private string? ExtractAttachmentMetadata(string htmlContent, string attachmentId, string fieldName)
    {
        try
        {
            // 匹配 <input> 或 <select> 中的 name="attachnew[attachmentId][fieldName]" 属性
            var pattern = $@"name=""attachnew\[{attachmentId}\]\[{fieldName}\]""[^>]*(?:value|checked)=""([^""]*)""";
            ...
            // 如果是 select 元素，查找 selected 的 option
            var selectPattern = $@"<select[^>]*name=""attachnew\[{attachmentId}\]\[{fieldName}\]""[^>]*>[\s\S]*?<option[^>]*value=""([^""]*)""[^>]*selected";
            var selectMatch = Regex.Match(htmlContent, selectPattern, RegexOptions.IgnoreCase | RegexOptions.Singleline);

            if (selectMatch.Success)
            {
                return selectMatch.Groups[1].Value;
            }
        ...
        {
            Debug.WriteLine($"⚠️ 提取元数据错误 (attachmentId={attachmentId}, field={fieldName}): {ex.Message}");
            return null;
        }
    }

    /// <summary>
    /// 辅助方法：从HTML中提取普通附件的文件名
    /// 查找格式：name="Fileinfo_AttachmentID_****" 或从 title 属性中提取
    /// </summary>
    private string ExtractAttachmentFileName(string htmlContent, string attachmentId)
    {
        try
        {
            // 尝试从 input 的 value 中获取文件名
            var fileNamePattern = $@"<input[^>]*name=""Fileinfo_AttachmentID_{attachmentId}""[^>]*value=""([^""]*)""";
            var fileNameMatch = Regex.Match(htmlContent, fileNamePattern, RegexOptions.IgnoreCase);
            ...
            // 尝试从相邻的文本节点中提取（可能在 <a> 标签内）
            var altPattern = $@"attachname{attachmentId}[^>]*>([^<]+)<";
            var altMatch = Regex.Match(htmlContent, altPattern, RegexOptions.IgnoreCase);
            ...
            return $"附件_{attachmentId}";
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"⚠️ 提取文件名错误 (attachmentId={attachmentId}): {ex.Message}");
            return $"附件_{attachmentId}";
        }
    }
    ...
    /// <summary>
    ///  新增：从HTML内容中提取分页信息
    /// 分页结构位于 &lt;div class="pgs mtm mbm cl"&gt; 中
    /// 
    /// 有分页的情况：
    ...
    /// 无分页的情况（只有一页）：
    /// &lt;div class="pgs mtm mbm cl"&gt;
    ///   &lt;span class="pgb y"&gt;&lt;a href="..."&gt;返回列表&lt;/a&gt;&lt;/span&gt;
    /// &lt;/div&gt;
    /// </summary>
    private PaginationInfo? ExtractPaginationInfo(string htmlContent)
    {
        try
        {
            // 查找分页容器 <div class="pgs mtm mbm cl">
            var paginationContainerPattern = @"<div[^>]*class=""[^""]*pgs[^""]*mtm[^""]*mbm[^""]*cl[^""]*""[^>]*>(.*?)</div>\s*(?=<div|<script|$)";
            var containerMatch = Regex.Match(htmlContent, paginationContainerPattern, RegexOptions.IgnoreCase | RegexOptions.Singleline);

            if (!containerMatch.Success)
            {
                Debug.WriteLine("⚠️ 未找到分页容器，假设这是单页帖子");
                return new PaginationInfo { CurrentPage = 1, TotalPages = 1 };
            }

            var paginationHtml = containerMatch.Groups[1].Value;

            // 检查是否有分页（<strong> 标签表示当前页）
            var currentPageMatch = Regex.Match(paginationHtml, @"<strong[^>]*>(\d+)</strong>", RegexOptions.IgnoreCase);

            if (!currentPageMatch.Success)
            {
                Debug.WriteLine("⚠️ 未找到当前页标记，假设这是单页帖子");
                return new PaginationInfo { CurrentPage = 1, TotalPages = 1 };
            }

            int currentPage = 1;
            if (int.TryParse(currentPageMatch.Groups[1].Value, out int page))
            {
                currentPage = page;
            }

            // 提取总页数 - 从 "/ 3 页" 或类似的标签中
            var totalPagesMatch = Regex.Match(paginationHtml, @"/\s*(\d+)\s*页", RegexOptions.IgnoreCase);
            int totalPages = 1;

            if (totalPagesMatch.Success)
            {
                if (int.TryParse(totalPagesMatch.Groups[1].Value, out int total))
                {
                    totalPages = total;
                }
            }
            ...
            {
                // 备选：统计页码链接的最大数字
                var pageLinksPattern = @"<a[^>]*href=""[^""]*&page=(\d+)""[^>]*>(\d+)</a>";
                var pageMatches = Regex.Matches(paginationHtml, pageLinksPattern, RegexOptions.IgnoreCase);
            ...
            Debug.WriteLine($"📄 分页信息已提取: 当前第 {currentPage} 页，共 {totalPages} 页");
        ...
        {
            Debug.WriteLine($"❌ 提取分页信息错误: {ex.Message}");
            return new PaginationInfo { CurrentPage = 1, TotalPages = 1 };
        }
    }
}
