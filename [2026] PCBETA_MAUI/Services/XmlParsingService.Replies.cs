...
public partial class XmlParsingService
{
    ...
    /// 注意：
    /// - 第一页（currentPage=1）：第一个 post 是楼主发帖，从第二个开始才是用户回帖
    /// - 第二页及以后：所有 post 都是用户回帖，从第一个开始加载
    /// </summary>
    private List<ReplyInfo>? ExtractRepliesFromHtml(string htmlContent, bool isFirstPage = true)
    {
        ...
        try
        {
            ...
            {
                // 没有任何post
                Debug.WriteLine("⏭️ 没有检测到任何post");
                return null;
            }

            // 确定起始索引：
            // - 如果是第一页，跳过第一个post（楼主），从第二个开始
            // - 如果不是第一页，从第一个post开始加载（都是用户回帖）
            int startIndex = isFirstPage ? 1 : 0;

            if (isFirstPage && postMatches.Count <= 1)
            {
                // 第一页只有楼主，没有回帖
                Debug.WriteLine("⏭️ 第一页没有检测到回帖（只有楼主）");
                return null;
            }

            // 从起始索引开始提取回帖
            for (int i = startIndex; i < postMatches.Count; i++)
            {
                ...
                int startIndexContent = postMatch.Index + postMatch.Length;
                ...
                else
                {
                    ...
                    var endMatch = Regex.Match(
                        htmlContent.Substring(startIndexContent),
                        @"<div[^>]*id=""postlistreply""",
                        RegexOptions.IgnoreCase
                    );
                    if (endMatch.Success)
                    {
                        endIndex = startIndexContent + endMatch.Index;
                    }
                }
    ...
    private ReplyInfo? ExtractReplyFromPost(string postHtml, string postId)
    {
        try
        {
            var reply = new ReplyInfo { ReplyId = postId };
            ...
            {
                reply.RatingUrl = ratingUrl;
                Debug.WriteLine($" 回帖评分URL: {ratingUrl}");
            }

            // ✅ 新增：11b. 提取编辑URL（用于编辑按钮）
            var editUrl = ExtractEditUrl(postHtml);
            if (!string.IsNullOrEmpty(editUrl))
            {
                reply.EditUrl = editUrl;
                reply.CanEdit = true;
                Debug.WriteLine($" 回帖编辑URL: {editUrl}");
            }

            // 12. 设置操作按钮的可见性
            reply.CanComment = !string.IsNullOrEmpty(messageHtml);
            reply.CanRate = !string.IsNullOrEmpty(messageHtml) && !string.IsNullOrEmpty(ratingUrl);
    ...
    /// <summary>
    /// ✅ 新增：从回帖HTML中提取评分URL
    /// 评分按钮格式：<a href="javascript:;" onclick="showWindow('rate', 'forum.php?mod=misc&action=rate&tid=2068108&pid=57924204'..."
    /// </summary>
    private string? ExtractRatingUrl(string postHtml)
    {
        try
        {
            // 匹配评分URL，从 onclick 属性中提取
            // 格式: onclick="showWindow('rate', 'forum.php?mod=misc&action=rate&tid=2068108&pid=57924204'..."
            var pattern = @"onclick=""showWindow\s*\(\s*'rate'\s*,\s*'([^']+)'";
            var match = Regex.Match(postHtml, pattern, RegexOptions.IgnoreCase);

            if (match.Success)
            {
                var ratingUrl = HtmlDecode(match.Groups[1].Value.Trim());
                Debug.WriteLine($"✅ 提取评分URL: {ratingUrl}");
                return ratingUrl;
            }

            Debug.WriteLine($"⚠️ 未能从HTML中提取评分URL");
            return null;
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"❌ 提取评分URL错误: {ex.Message}");
            return null;
        }
    }
    ...
    /// <summary>
    /// ✅ 新增：从回帖HTML中提取编辑URL
    /// 编辑按钮格式：<a class="editp" href="forum.php?mod=post&action=edit&fid=16&tid=2069511&pid=57955816&page=1">编辑</a>
    /// </summary>
    private string? ExtractEditUrl(string postHtml)
    {
        try
        {
            // 匹配编辑按钮，从 href 属性中提取 URL
            // 格式: <a class="editp" href="forum.php?mod=post&action=edit&fid=16&tid=2069511&pid=57955816&page=1">编辑</a>
            var pattern = @"<a\s+class=""editp""[^>]*href=""([^""]+)""[^>]*>";
            var match = Regex.Match(postHtml, pattern, RegexOptions.IgnoreCase);

            if (match.Success)
            {
                var editUrl = HtmlDecode(match.Groups[1].Value.Trim());
                Debug.WriteLine($"✅ 提取编辑URL: {editUrl}");
                return editUrl;
            }

            Debug.WriteLine($"⚠️ 未能从HTML中提取编辑URL");
            return null;
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"❌ 提取编辑URL错误: {ex.Message}");
            return null;
        }
    }
}
