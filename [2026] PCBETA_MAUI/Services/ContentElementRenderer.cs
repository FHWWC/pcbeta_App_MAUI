...
public static class ContentElementRenderer
{
    ...
    /// 链接点击事件处理
    /// 支持：
    /// 1. 论坛内部链接（viewthread）- 导航到 ThreadContentPage
    /// 2. 论坛板块链接（forum）- 导航到 ThreadListPage
    /// 3. 可下载资源 - 使用 FileSaver
    /// 4. 其他链接 - 使用浏览器打开
    /// </summary>
    private static async Task OnLinkTapped(string? url, string? title)
    {
        ...
        {
            // 检查是否为论坛内部链接
            if (IsInternalForumLink(url, out var linkType, out var linkParams))
            {
                await HandleInternalForumLink(linkType, linkParams);
                return;
            }
    ...
    /// 检查是否为论坛内部链接
    /// </summary>
    private static bool IsInternalForumLink(string url, out string linkType, out Dictionary<string, string> parameters)
    {
        ...
        // 检查是否为viewthread链接（查看帖子）
        // 格式1: https://bbs.pcbeta.com/viewthread-XXXXX-X-X.html
        // 格式2: https://bbs.pcbeta.com/forum.php?mod=viewthread&tid=XXXXXX
        if (lowerUrl.Contains("viewthread"))
        {
            linkType = "viewthread";
            ExtractViewThreadParams(url, parameters);
            return !string.IsNullOrEmpty(parameters.ContainsKey("tid") ? parameters["tid"] : null);
        }

        // 检查是否为forum链接（查看板块）
        // 格式: https://bbs.pcbeta.com/forum-XXX-X.html 或 https://bbs.pcbeta.com/forum-HarmonyOS-1.html
        if (lowerUrl.Contains("forum-") && !lowerUrl.Contains("viewthread"))
        {
            linkType = "forum";
            ExtractForumParams(url, parameters);
            return !string.IsNullOrEmpty(parameters.ContainsKey("fid") ? parameters["fid"] : null);
        }
    ...
    /// 提取 viewthread 链接的参数
    /// </summary>
    private static void ExtractViewThreadParams(string url, Dictionary<string, string> parameters)
    {
        try
        {
            var lowerUrl = url.ToLower();

            // 尝试从查询字符串中提取 tid（格式: ?mod=viewthread&tid=2069023）
            ...
            if (questionIndex > -1)
            {
                ...
                if (queryParams.TryGetValue("tid", out var tid))
                {
                    parameters["tid"] = tid;
                    if (queryParams.TryGetValue("page", out var page))
                        parameters["page"] = page;
                    Debug.WriteLine($"🔗 提取 viewthread 参数（查询字符串）: tid={tid}, page={page ?? "1"}");
                    return;
                }
            }

            // 尝试从路径中提取 tid（格式: viewthread-2069023-X-X.html）
            var match = Regex.Match(lowerUrl, @"viewthread[/-](\d+)[/-]", RegexOptions.IgnoreCase);
            if (match.Success && match.Groups.Count > 1)
            {
                parameters["tid"] = match.Groups[1].Value;
                Debug.WriteLine($"🔗 提取 viewthread 参数（路径）: tid={match.Groups[1].Value}");
                return;
            }
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"⚠️ 提取 viewthread 参数失败: {ex.Message}");
        }
    }
    ...
    /// 提取 forum 链接的参数
    /// </summary>
    private static void ExtractForumParams(string url, Dictionary<string, string> parameters)
    {
        try
        {
            var lowerUrl = url.ToLower();

            // 格式: forum-573-1.html 或 forum-HarmonyOS-1.html
            // 提取第一个参数作为 fid（可能是数字或名称）
            var match = Regex.Match(lowerUrl, @"forum[/-]([^/-]+)[/-]", RegexOptions.IgnoreCase);
            if (match.Success && match.Groups.Count > 1)
            {
                var forumId = match.Groups[1].Value;
                parameters["fid"] = forumId;

                // 尝试提取页码
                var pageMatch = Regex.Match(lowerUrl, @"forum[/-][^/-]+[/-](\d+)");
                if (pageMatch.Success && pageMatch.Groups.Count > 1)
                {
                    parameters["page"] = pageMatch.Groups[1].Value;
                }

                var pageValue = parameters.ContainsKey("page") ? parameters["page"] : "1";
                Debug.WriteLine($"🔗 提取 forum 参数: fid={forumId}, page={pageValue}");
                return;
            }
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"⚠️ 提取 forum 参数失败: {ex.Message}");
        }
    }
    ...
    /// 处理论坛内部链接导航
    /// </summary>
    private static async Task HandleInternalForumLink(string linkType, Dictionary<string, string> parameters)
    {
        try
        {
            ...
            if (linkType == "viewthread")
            {
                // 导航到 ThreadContentPage
                if (parameters.TryGetValue("tid", out var tid))
                {
                    var page = parameters.ContainsKey("page") ? parameters["page"] : "1";
                    Debug.WriteLine($"📄 导航到 viewthread: tid={tid}, page={page}");

                    // 获取帖子信息以获取标题
                    try
                    {
                        int pageNum = int.Parse(page);
                        var threadContent = await apiService.GetThreadContentAsync(tid, pageNum);
                        var threadTitle = threadContent?.Title ?? $"Thread {tid}";

                        await navigationService.NavigateToAsync("threadcontent", new Dictionary<string, object>
                        {
                            { "threadId", tid },
                    ...
                    {
                        Debug.WriteLine($"⚠️ 获取帖子信息失败: {ex.Message}，使用默认标题");
                        await navigationService.NavigateToAsync("threadcontent", new Dictionary<string, object>
                        {
                            { "threadId", tid },
                            { "threadTitle", $"Thread {tid}" }
                        });
                    }
                }
            }
            else if (linkType == "forum")
            {
                // 导航到 ThreadListPage
                if (parameters.TryGetValue("fid", out var fid))
                {
                    var page = parameters.ContainsKey("page") ? parameters["page"] : "1";
                    Debug.WriteLine($"📋 导航到 forum: fid={fid}, page={page}");

                    // 获取板块信息
                    try
                    {
                        int pageNum = int.Parse(page);
                        var forumName = $"Forum {fid}";  // 默认名称

                        await navigationService.NavigateToAsync("threadlist", new Dictionary<string, object>
                        {
                            { "boardId", fid },
                            { "forumName", forumName },
                            { "page", pageNum }
                    ...
                    {
                        Debug.WriteLine($"⚠️ 导航到板块失败: {ex.Message}");
                        await Application.Current?.Windows[0].Page?.DisplayAlertAsync("错误", $"无法打开板块: {ex.Message}", "确定");
                    }
                }
            }
        }
        ...
        {
            Debug.WriteLine($"❌ 处理论坛内部链接失败: {ex.Message}");
            await Application.Current?.Windows[0].Page?.DisplayAlertAsync("错误", $"链接处理失败: {ex.Message}", "确定");
        }
    }
